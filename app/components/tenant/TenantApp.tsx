  ...
  return (
    <div className="app-shell tenant-shell">
      <aside className={`sidebar ${menuOpen ? "sidebar-open" : ""} ${sidebarPinned ? "sidebar-pinned" : ""}`}>
        ...
        <div className={state.portal.logo ? "workspace-switcher has-logo" : "workspace-switcher"}>{state.portal.logo ? <img src={state.portal.logo} alt={state.portal.storeName} className="workspace-logo" /> : <><span className="avatar avatar-coral">{state.portal.storeName.split(" ").map((word) => word[0]).slice(0, 2).join("")}</span><span className="sidebar-copy"><small>Mi emprendimiento</small><strong>{state.portal.storeName}</strong></span></>}</div>
