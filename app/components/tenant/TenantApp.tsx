  ...
  return (
    <div className="app-shell tenant-shell">
      <aside className={`sidebar ${menuOpen ? "sidebar-open" : ""}`}>
        <button className="brand" onClick={() => navigate("inicio")} aria-label="Ir al inicio"><span className="brand-mark">N</span><span><strong>Nexo</strong><small>gestión de negocio</small></span></button>
        <div className="workspace-switcher"><span className="avatar avatar-coral">LC</span><span><small>Mi emprendimiento</small><strong>Luna Creativa</strong></span><span className="chevron">⌄</span></div>
        <nav aria-label="Navegación del emprendimiento">
          <p className="nav-label">Gestión</p>
          {tenantNavItems.map((item) => <button key={item.id} className={active === item.id ? "nav-item active" : "nav-item"} onClick={() => navigate(item.id)}><span className="nav-glyph">{item.glyph}</span>{item.label}{item.id === "pedidos" && <span className="nav-count">{state.orders.filter((order) => order.status === "Nuevo").length}</span>}{item.id === "inventario" && lowStock > 0 && <span className="nav-alert">{lowStock}</span>}</button>)}
          <p className="nav-label nav-label-system">Negocio</p>
          <button className={active === "ajustes" ? "nav-item active" : "nav-item"} onClick={() => navigate("ajustes")}><span className="nav-glyph">A</span>Ajustes</button>
        </nav>
        ...
        <div className="sidebar-footer"><div className="mini-profile"><span className="avatar avatar-dark">MS</span><span><strong>Marina Suárez</strong><small>Administradora</small></span></div><button className="dots" onClick={onExit}>Salir</button></div>
      </aside>
...
function TenantSettings({ resetDemo }: { resetDemo: () => void }) {
  return <><div className="page-heading"><div><p className="eyebrow">Mi negocio</p><h1>Ajustes</h1><p>Preferencias de la demostración y próximos accesos del equipo.</p></div></div><div className="settings-grid"><section className="panel settings-card"><span className="settings-icon">DE</span><div><h2>Datos de demostración</h2><p>Volvé a cargar productos, clientes, proveedores y pedidos originales.</p><button className="button secondary" onClick={resetDemo}>Restablecer demostración</button></div></section><section className="panel settings-card"><span className="settings-icon">US</span><div><h2>Usuarios del espacio</h2><p>Tu plan actual admite una administradora. La gestión de empleados llegará en una próxima versión.</p><button className="button secondary" disabled>1 de 1 usuario</button></div></section><section className="panel settings-card"><span className="settings-icon">BD</span><div><h2>Conexión definitiva</h2><p>Esta versión prueba los flujos sin mezclar datos con la futura base productiva.</p><button className="button secondary" disabled>Supabase pendiente</button></div></section></div></>;
}
