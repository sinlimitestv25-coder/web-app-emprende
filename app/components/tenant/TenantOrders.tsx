...
export function TenantOrders({ state, setState, changeStatus, flash }: { state: TenantDemoState; setState: Dispatch<SetStateAction<TenantDemoState>>; changeStatus: (id: string, status: OrderStatus) => void; flash: (message: string) => void }) {
  ...
  return <>
    ...
    <div className="order-pipeline">
      <button className={filter === "Todos" ? "stage active" : "stage"} onClick={() => setFilter("Todos")}><strong>{state.orders.length}</strong><span>Todos</span></button>
      {statuses.map((status) => <button className={filter === status ? "stage active" : "stage"} key={status} onClick={() => setFilter(status)}><strong>{state.orders.filter((order) => order.status === status).length}</strong><span>{status}</span></button>)}
    </div>
