...
export function TenantOrders({ state, setState, changeStatus, flash }: { state: TenantDemoState; setState: Dispatch<SetStateAction<TenantDemoState>>; changeStatus: (id: string, status: OrderStatus) => void; flash: (message: string) => void }) {
  ...
  function save(event: FormEvent) {
    ...
    const order: Order = { id: `PED-${1049 + state.orders.length}`, customerId: customer.id, customerName: customer.name, items: [{ productId: product.id, productName: product.name, quantity, unitPrice: product.price }], total: product.price * quantity, status: "Nuevo", createdAt: "Ahora", stockCommitted: false };
    ...
    <section className="panel orders-table">
      ...
      {visible.map((order) => (
        <div className="tenant-orders-row" key={order.id}>
          <strong>{order.id}<small>{order.createdAt}</small></strong>
          ...
          <div className="order-detail-cell">
            {order.items.length === 1
              ? <><span>{order.items[0].productName}</span><small>{order.items[0].quantity} × {money.format(order.items[0].unitPrice)} {order.stockCommitted ? "· Stock reservado" : ""}</small></>
        ...
        <div className="cart-lines">
          {order.items.map((item) => (
            <div key={item.productId}>
              <span><strong>{item.productName}</strong><small>{item.quantity} × {money.format(item.unitPrice)}</small></span>
