...
export function TenantContacts({ mode, state, setState, flash }: Props) {
  ...
  return <>
    ...
    <div className="summary-strip contact-summary"><div><span>{isCustomers ? "Clientes registrados" : "Proveedores activos"}</span><strong>{records.length}</strong></div><div><span>{isCustomers ? "Con pedidos" : "Rubros cubiertos"}</span><strong>{isCustomers ? new Set(state.orders.map((order) => order.customerId)).size : state.suppliers.length}</strong></div><div><span>{isCustomers ? "Pedidos históricos" : "Contactos disponibles"}</span><strong>{isCustomers ? state.orders.length : state.suppliers.filter((supplier) => supplier.phone).length}</strong></div></div>
