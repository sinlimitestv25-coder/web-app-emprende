  ...
  return <>
    ...
    <section className="panel contacts-panel table-panel">
      ...
      {visible.map((record) => isCustomers
        ? <CustomerRow key={record.id} customer={record as Customer} orders={state.orders.filter((order) => order.customerId === record.id).length} />
        : <SupplierRow key={record.id} supplier={record as Supplier} />)}
...
function CustomerRow({ customer, orders }: { customer: Customer; orders: number }) {
  return <div className="contacts-row">
    ...
    <div className="row-actions"><a className="whatsapp-icon-button" href={`https://wa.me/${customer.phone.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" aria-label={`Abrir WhatsApp con ${customer.name}`}><AppIcon name="whatsapp" /></a></div>
  </div>;
}
...
function SupplierRow({ supplier }: { supplier: Supplier }) {
  return <div className="contacts-row">
    ...
    <div className="row-actions"><a className="whatsapp-icon-button" href={`https://wa.me/${supplier.phone.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" aria-label={`Abrir WhatsApp con ${supplier.name}`}><AppIcon name="whatsapp" /></a></div>
  </div>;
}
