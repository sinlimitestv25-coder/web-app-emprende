...
export function Spaces({ demoMode }: { demoMode: boolean }) {
  ...
  return (
    <>
      ...
      {selected && <SpaceEditModal space={selected} onClose={() => setSelected(null)} onUpdate={updateDetails} />}
    </>
  );
}
...
function SpaceEditModal({ space, onClose, onUpdate }: { space: PlatformSpace; onClose: () => void; onUpdate: (id: string, patch: { name: string; plan: PlatformSpace["plan"]; maxUsers: number }) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><form className="modal space-edit-modal" onSubmit={save} onMouseDown={(event) => event.stopPropagation()}>
    <button type="button" className="modal-close" onClick={onClose} aria-label="Cerrar">×</button>
    <div className="drawer-identity"><span className={`avatar avatar-${space.tone}`}>{space.initials}</span><div><p>{space.id}</p><h2>{space.name}</h2><span className={`status ${space.status === "Activo" ? "success" : "neutral"}`}>{space.status}</span></div></div>
    ...
    <section><h3>Administración</h3><dl><div><dt>Responsable</dt><dd>{space.owner}</dd></div><div><dt>Correo</dt><dd>{space.email}</dd></div><div><dt>Usuarios</dt><dd>{space.users} de {space.maxUsers}</dd></div><div><dt>Uso</dt><dd>{space.storage}</dd></div><div><dt>Último acceso</dt><dd>{space.lastAccess}</dd></div></dl></section>
    ...
    <div className="wizard-buttons"><button type="button" className="button secondary" onClick={onClose}>Cancelar</button><button className="button primary">Guardar cambios</button></div>
  </form></div>;
}
